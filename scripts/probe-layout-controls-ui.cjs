...
const directory=fs.mkdtempSync(path.resolve('.runtime/tests/layout-controls-ui-'));app.setPath('userData',directory);app.disableHardwareAcceleration();
...
const timer=setTimeout(()=>{record.error='timeout';fs.writeFileSync('validation/layout-controls-ui.json',JSON.stringify(record,null,2));app.exit(1)},45000);let started=false;
app.on('browser-window-created',(_,win)=>{if(started)return;started=true;win.webContents.once('did-finish-load',()=>void(async()=>{
 ...
 clearTimeout(timer);fs.writeFileSync('validation/layout-controls-ui.json',JSON.stringify(record,null,2));app.quit();
})().catch(error=>{record.error=error.stack;clearTimeout(timer);fs.writeFileSync('validation/layout-controls-ui.json',JSON.stringify(record,null,2));app.exit(1)}));});
