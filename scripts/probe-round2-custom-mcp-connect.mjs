...
try{
  ...
  record.passed=true;
}catch(e){record.error=e.message}finally{await transport?.stop();httpServer.closeAllConnections();await new Promise(resolve=>httpServer.close(resolve));await fs.writeFile('validation/round2-custom-mcp-connect.json',JSON.stringify(record,null,2));console.log(JSON.stringify(record));if(!record.passed)process.exitCode=1}
