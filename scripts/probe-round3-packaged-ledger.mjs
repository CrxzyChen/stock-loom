...
// Prepare a current-format fixture; ledger operations below use the frozen service.
...
assert s.db.execute('PRAGMA user_version').fetchone()[0]==10
...
try{
 ...
 let ledger=await service.call('ledger.read',{instrumentId:'600000.SH'});assert.equal(ledger.quantity,100);assert.equal(ledger.events[0].event.kind,'balance');result.checks.push('frozen service reads current recorded balance without inventing a trade');
