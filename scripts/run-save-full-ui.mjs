...
const result=spawnSync(electron,[path.resolve('scripts/probe-save-full-ui.cjs'),directory],{stdio:'inherit',timeout:40000,windowsHide:true});assert.equal(result.status,0);
const stage=JSON.parse(fs.readFileSync(path.join(directory,'result.json')));assert.equal(stage.passed,true,stage.error);
const record={createdAt:new Date().toISOString(),directory,...stage};fs.writeFileSync('validation/save-full-ui-probe.json',JSON.stringify(record,null,2));console.log(JSON.stringify(record));
