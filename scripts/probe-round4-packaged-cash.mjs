...
try{await service.start();assert.equal((await service.call('overview')).schemaVersion,10);assert.equal((await service.call('cash.read',{})).balance,null);assert.equal((await fs.readdir(path.join(dir,'backups'))).length,0);result.currentSchema=true;
