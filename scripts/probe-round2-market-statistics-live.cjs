...
app.whenReady().then(async()=>{
  ...
  for(const marketId of ['SH_A','SZ_A','SH_STAR','SZ_GEM']){
    ...
    fs.writeFileSync('validation/round2-market-statistics-live.json',JSON.stringify(record,null,2));
  }
  ...
  fs.writeFileSync('validation/round2-market-statistics-live.json',JSON.stringify(record,null,2));app.exit(record.completed?0:1);
});
