...
const directory=fs.mkdtempSync(path.resolve('.runtime/tests/icon-requests-ui-'));app.setPath('userData',directory);app.disableHardwareAcceleration();
...
function finish(error){clearTimeout(timer);if(error)record.error=error.stack;else record.passed=true;fs.writeFileSync('validation/icon-requests-ui.json',JSON.stringify(record,null,2));app.exit(error?1:0)}
app.on('browser-window-created',(_,win)=>{if(started)return;started=true;win.webContents.once('did-finish-load',()=>void(async()=>{
 ...
 await wait(`document.querySelector('.history-thread')`);await js(`document.querySelectorAll('.quick-layout button')[1].click();document.querySelector('.history-thread').click()`);
