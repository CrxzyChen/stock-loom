...
app.whenReady().then(async()=>{
  ...
  for(const endpoint of ['index_daily','daily_info','sz_daily_info','anns_d']){
    ...
    fs.writeFileSync('validation/round2-source-diagnostics.json',JSON.stringify(record,null,2));
  }
  record.completed=true;
}).catch(()=>{record.error='诊断未完成；请检查本地凭证状态与服务连接。'}).finally(async()=>{
  await service?.stop();await guard?.stop();
  fs.writeFileSync('validation/round2-source-diagnostics.json',JSON.stringify(record,null,2));app.exit(record.completed?0:1);
});
