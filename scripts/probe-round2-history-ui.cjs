...
const directory=fs.mkdtempSync(path.resolve('.runtime/tests/round2-history-ui-'));app.setPath('userData',directory);app.disableHardwareAcceleration();
...
const timer=setTimeout(()=>{record.error='timeout';fs.writeFileSync('validation/round2-history-ui.json',JSON.stringify(record,null,2));app.exit(1)},45000);let started=false;
app.on('browser-window-created',(_,win)=>{if(started)return;started=true;win.webContents.once('did-finish-load',()=>void(async()=>{
 ...
 await click('历史');await wait(`document.querySelector('[aria-label="选择对话"]')?.options.length>1`);
 await js(`(()=>{const s=document.querySelector('[aria-label="选择对话"]');s.value=${JSON.stringify(history.thread.id)};s.dispatchEvent(new Event('change',{bubbles:true}))})()`);
 ...
 await wait(`document.querySelector('.copilot-messages').textContent.includes('48271')`);
 assert.equal(await js(`document.querySelector('[aria-label="选择对话"]').value`),history.thread.id);record.selectedConversationRestored=true;
 ...
 assert.ok(await js('document.documentElement.scrollWidth<=innerWidth'));record.passed=true;
 clearTimeout(timer);fs.writeFileSync('validation/round2-history-ui.json',JSON.stringify(record,null,2));app.quit();
})().catch(error=>{record.error=error.stack;clearTimeout(timer);fs.writeFileSync('validation/round2-history-ui.json',JSON.stringify(record,null,2));app.exit(1)}));});
