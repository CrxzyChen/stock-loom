...
try{
  ...
  record.passed=true;
}catch(e){record.error=e.message}finally{await transport.stop();await pipe.close();await service.stop();await fs.writeFile('validation/round2-mcp.json',JSON.stringify(record,null,2));console.log(JSON.stringify(record));if(!record.passed)process.exitCode=1}
