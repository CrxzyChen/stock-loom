...
import {verifyComputerUse} from './verify-computer-use.mjs';
...
for(const script of ['build-desktop.mjs','build-service.mjs','build-computer-use.mjs','build-notices.mjs']){
...
const computerUse=JSON.parse(fs.readFileSync('build/computer-use-current.json','utf8'));verifyComputerUse(computerUse);
...
// Explicit files bypass electron-builder's default node_modules / dotfile filters.
// The independently hashed build manifest is also verified after copying.
for(const entry of computerUse.files){
  const [kind,...parts]=entry.file.split('/');
  config.extraResources.push({from:path.join(kind==='native'?computerUse.native:computerUse.javascript,...parts),to:[kind==='native'?'computer-use-native':'computer-use',...parts].join('/')});
}
...
const computerUseVerification=verifyComputerUse(computerUse,{native:path.join(output,'win-unpacked/resources/computer-use-native'),javascript:path.join(output,'win-unpacked/resources/computer-use')});
fs.writeFileSync(path.join(output,'computer-use-verification.json'),JSON.stringify(computerUseVerification,null,2));
...
fs.writeFileSync('build/package-current.json',JSON.stringify({createdAt:new Date().toISOString(),directory:output,service:record,desktop,computerUse},null,2));
