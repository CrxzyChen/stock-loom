...
def index_rows(code,start,end,raw,*,require_trading_days=True):
    ...
    for source in raw:
        ...
        if require_trading_days and dt.datetime.strptime(day,'%Y%m%d').weekday()>=5:raise ProviderError('INVALID_DATA','指数行情包含非交易日。')
...
class IndexData:
    def validate_index_calendar(self,rows):
        closed={r[0] for r in self.db.execute("SELECT cal_date FROM trading_calendar WHERE exchange='SSE' AND is_open=0 AND cal_date BETWEEN ? AND ?",(rows[0]['date'],rows[-1]['date']))}
        if any(r['date'] in closed for r in rows):raise ProviderError('INVALID_DATA','指数行情包含非交易日。')
    ...
    def sync_index(self,p,fetch=query):
        ...
        self.validate_index_calendar(rows)
    ...
    def checked_index(self,record,*,for_archive=False):
        try:
            ...
            # Archiving preserves legacy source records, including calendar anomalies.
            # Hash, schema and numeric validation still apply; normal reads remain strict.
            rows=index_rows(payload['indexId'],payload['start'],payload['end'],payload['source'],require_trading_days=not for_archive)
            if not for_archive:self.validate_index_calendar(rows)
    ...
    def read_index(self,p):
        ...
        if 'snapshotId' in p:
            ...
            # Retain invalid versions for inspection, but do not let them mask valid data.
            invalid=False
            for candidate in self.db.execute('SELECT id,manifest FROM snapshots WHERE dataset=? ORDER BY as_of DESC,rowid DESC',('index:'+p['indexId'],)):
                try:
                    result=self.checked_index(candidate)
                    if result['indexId']!=p['indexId']:raise ProviderError('CORRUPT_SNAPSHOT','指数归属不一致。')
                    return result
                except ProviderError:invalid=True
            if invalid:raise ProviderError('CORRUPT_SNAPSHOT','指数快照校验失败，已有资料保留。')
            return None
