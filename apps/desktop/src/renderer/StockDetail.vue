...
const updating=ref(false);async function update(){if(!window.stock||updating.value)return;updating.value=true;error.value='';try{const results=await Promise.all(['bars','daily_basic'].map(endpoint=>window.stock!.ensureData({instrumentId:props.id,endpoint:endpoint as 'bars'|'daily_basic',years:1,force:true})));const failed=results.find(r=>r.state==='failed');if(failed)error.value=connectionError(failed.message,'data');window.dispatchEvent(new Event('stock-data-updated'))}catch(e){error.value=connectionError(e,'data')}finally{updating.value=false}}
