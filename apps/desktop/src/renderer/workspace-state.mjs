const pages=new Set(['holdings','market','watchlists','research','jobs','settings']);
...
function validTab(id){return typeof id==='string'&&(pages.has(id)||comparison(id)||/^stock:[A-Z0-9]{6,12}\.(SH|SZ|BJ)$/.test(id)||(id.startsWith('file:')&&id.length>5&&id.length<2000&&!id.slice(5).split(/[\\/]/).some(s=>s==='..')&&!/^[\\/]|:/.test(id.slice(5))))}
...
export function readWorkspace(storage,project){
  ...
  try{
    ...
    const tabs=[...new Set(value.tabs.map(id=>id==='screen'?'market':id).filter(validTab))].slice(0,30);
    return {tabs,active:tabs.includes(value.active==='screen'?'market':value.active)?(value.active==='screen'?'market':value.active):tabs[0]??null,panel:panels.has(value.panel)?value.panel:'market'};
