...
export function useStockData(input:()=>{instrumentId:string;endpoint:'bars'|'daily_basic'|'income'|'balancesheet'|'cashflow';years:1|3|4},updated:()=>Promise<unknown>){
 ...
 catch(e){if(!closed&&id===generation){state.value='failed';message.value=String(e).replace(/^Error invoking remote method '[^']+': Error: /,'')}}
 ...
 const update=()=>{void updated();void check()};window.addEventListener('stock-data-updated',update);
