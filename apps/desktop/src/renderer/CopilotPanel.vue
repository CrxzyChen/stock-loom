...
async function open(id:string){if(!api)return;const result=await api.copilotRead(id);if(result.unavailable){threads.value=threads.value.filter(t=>t.id!==id);if(threadId.value===id){threadId.value='';items.value=[];requests.value=[];running.value=false}try{if(localStorage.getItem('stock:copilot:selected:'+project.value)===id)localStorage.removeItem('stock:copilot:selected:'+project.value)}catch{}error.value='之前选择的对话当前不可读取。未发送的空对话可能不会保留；草稿仍在，可新建对话或从历史选择。';return}threadId.value=id;requests.value=result.pendingRequests??[];items.value=(result.thread.turns??[]).flatMap((t:any)=>(t.items??[]).map((i:any)=>({...i,_turnId:t.id})));historyCursor.value=result.historyNextCursor??null;historyUnavailable.value=!!result.historyUnavailable;running.value=result.thread.status?.type==='active';if(!threads.value.some(t=>t.id===id))threads.value.unshift(result.thread);remember()}
async function locateTurn(id:string){
 const visited=new Set<string>();await nextTick();followBottom.value=false;
 while(!items.value.some(i=>i._turnId===id)&&historyCursor.value&&!visited.has(historyCursor.value)){
  visited.add(historyCursor.value);await older();
 }
 await nextTick();const element=Array.from(messages.value?.querySelectorAll<HTMLElement>('[data-turn-id]')??[]).find(el=>el.dataset.turnId===id);
 if(element){element.scrollIntoView({block:'start'});lastScrollTop=messages.value?.scrollTop??0;measureBottom()}
 else error.value='这次运行的历史内容当前不可读取。';
}
async function older(){if(!api||!historyCursor.value)return;followBottom.value=false;const result=await api.copilotRead(threadId.value,historyCursor.value);if(result.historyUnavailable)throw Error('当前连接无法加载更早消息。');const height=messages.value?.scrollHeight??0,top=messages.value?.scrollTop??0;const existing=new Set(items.value.map(i=>i.id));items.value=[...(result.thread.turns??[]).flatMap((t:any)=>(t.items??[]).map((i:any)=>({...i,_turnId:t.id}))).filter((i:any)=>!existing.has(i.id)),...items.value];historyCursor.value=result.historyNextCursor??null;await nextTick();if(messages.value){messages.value.scrollTop=top+messages.value.scrollHeight-height;lastScrollTop=messages.value.scrollTop;measureBottom()}}
