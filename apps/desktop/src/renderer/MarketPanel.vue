...
async function search(){if(!window.stock||closed)return;const id=++searchId;try{const data=await window.stock.searchInstruments(query.value,0);if(id===searchId){matches.value=data.items;searched.value=true}}catch(e){if(id===searchId)error.value=String(e instanceof Error?e.message:e)}}
async function choose(item:Instrument){
  ...
  catch(e){if(id===selectionId)error.value=String(e instanceof Error?e.message:e)}
  finally{if(id===selectionId)listing.value=false}
}
watch(()=>props.instrumentId,async id=>{if(!id||!window.stock)return;try{const result=await window.stock.searchInstruments(id,0);const item=result.items.find(item=>item.id===id);if(item)await choose(item);else error.value='本地目录没有这只股票，请先同步目录。'}catch(e){error.value=e instanceof Error?e.message:String(e)}},{immediate:true});
watch([snapshot,adjustment],async()=>{
  ...
  }catch(e){if(id===readId)error.value=String(e instanceof Error?e.message:e)}finally{if(id===readId)loading.value=false}
});
async function sync(){
  ...
  catch(e){if(!closed&&selected.value?.id===item.id)error.value=String(e instanceof Error?e.message:e)}finally{if(!closed)busy.value=false}
}
const requestedYears=ref<1|3|4>(chartDefaults.range===10000?3:1);
async function refreshVersions(){if(!window.stock||!selected.value||closed)return;const code=selected.value.id,previousLatest=versions.value[0]?.snapshotId;const rows=await window.stock.barVersions(code);if(closed||selected.value?.id!==code)return;const followLatest=!snapshot.value||snapshot.value===previousLatest;versions.value=rows;if(followLatest&&rows[0]&&snapshot.value!==rows[0].snapshotId){snapshot.value=rows[0].snapshotId;emit('changed')}}
