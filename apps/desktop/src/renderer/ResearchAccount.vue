...
const desktop=Boolean(window.stock),busy=ref(false),error=ref(''),mode=ref<'chatgpt'|'api'|'custom'>('chatgpt');
...
const apiModel=ref(''),apiKey=ref(''),apiReady=ref(false),saved=ref(false);
let disposed=false,polling=false,timer:ReturnType<typeof setInterval>|undefined;
watch([mode,account,apiReady,customReady],()=>emit('ready',mode.value==='custom'?customReady.value:mode.value==='api'?apiReady.value:account.value.connected&&!account.value.pending&&!!account.value.model),{immediate:true,deep:true});
...
onBeforeUnmount(()=>{disposed=true;clearInterval(timer);apiKey.value=''});
</script>
<template>
  <section class="account-section" aria-labelledby="account-heading">
    ...
    <div v-if="mode==='api'" class="legacy-api"><p>OpenAI · {{apiReady?'已配置':'未配置'}}</p><p>{{apiModel}}</p><UiButton icon="edit" :disabled="busy||running" @click="switchMode('custom')">更换服务</UiButton></div>
  </section>
</template>
<style scoped>
...
.connection-segments{display:flex;gap:4px;margin:20px 0 24px}.connection-segments button{min-width:100px}.legacy-api{display:grid;gap:12px}
