...
export function nextTaskDue(task,date){
 if(!task.enabled)return null;
 if(task.frequency==='cron'||task.frequency==='interval')return task.nextDue??(task.frequency==='cron'?nextCron(task,date):date.getTime()+task.intervalSeconds*1000);
...
export class TaskScheduler{
 ...
 let status;if(event.kind==='request')status='waiting';else if(event.method==='serverRequest/resolved')status=p.pendingRequests>0?'waiting':'running';else if(event.method==='turn/started')status='running';else if(event.method==='turn/completed')status=p.turn?.error||p.turn?.status==='failed'?'failed':p.turn?.status==='interrupted'?'interrupted':'succeeded';if(!status)return;
 void this.locked(async()=>{if(p.turn?.id&&record.turnId&&p.turn.id!==record.turnId&&event.method!=='turn/started')return;if(event.method==='turn/started'&&p.turn?.id)record.turnId=p.turn.id;record.status=status;if(status==='failed')record.message='本轮执行失败，打开会话查看详情。';if(status==='interrupted')record.message='本轮已停止，可手动重试。';if(['failed','interrupted','succeeded'].includes(status))record.finishedAt=this.now().toISOString();await this.persist()}).catch(e=>{this.error=e.message});
 }
