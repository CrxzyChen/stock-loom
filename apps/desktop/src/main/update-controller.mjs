...
import {repository,checkUpdate,downloadUpdate} from './updates.mjs';
...
export class UpdateController{
  ...
  constructor({directory,current,getSchema,check=checkUpdate,download=downloadUpdate,verify=async()=>({verified:false,reason:'未配置发布签名校验。'})}){
    ...
    this.state={repo:'',current,state:'idle',version:null,notes:'',received:0,total:0,message:'尚未配置发布源'};
  ...
  async configure(repo){
    ...
    const operation=(async()=>{
      ...
      await fs.writeFile(temporary,JSON.stringify({repo}),{flag:'wx'});await fs.rename(temporary,file);
      this.update=null;this.downloaded=null;this.state={repo,current:this.current,state:'idle',version:null,notes:'',received:0,total:0,message:repo?'发布源已保存，请手动检查':'已停用更新检查'};
    })();
  ...
  async run(kind){
    ...
    const operation=(async()=>{
      ...
      if(kind==='check'){
        const result=await this.checkImpl({repo:this.state.repo,current:this.current,schema,signal});signal.throwIfAborted();
        this.update=result.available?result.update:null;this.downloaded=null;
        Object.assign(this.state,{state:result.available?'available':'current',version:result.update.version,notes:result.update.notes,received:0,total:result.update.size,message:result.available?'发现新版本':'当前没有更新版本'});
      }else{
