...
for(const [name,description,method,inputSchema] of workspaceTools){
 ...
 const mutation=sync||['scheduler.notify','scheduler.save','scheduler.remove','watchlists.create','watchlists.add','watchlists.remove','watchlists.rename','holdings.save','ledger.write','ledger.import','cash.write'].includes(method);
