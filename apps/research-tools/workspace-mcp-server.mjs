...
for(const [name,description,method,inputSchema] of workspaceTools){
 ...
 const mutation=sync||['scheduler.save','scheduler.remove','watchlists.create','watchlists.add','watchlists.remove','watchlists.rename','holdings.save'].includes(method);
 server.registerTool(name,{description,inputSchema,annotations:{readOnlyHint:!mutation,destructiveHint:method==='scheduler.remove'||method==='watchlists.remove'||method==='holdings.save',openWorldHint:!!sync}},async args=>{
